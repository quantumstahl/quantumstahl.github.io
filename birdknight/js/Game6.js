      ...
      for (let oti=0; oti<layer.objectype.length; oti++){
        ...
        for (let oi=0; oi<ot.objects.length; oi++){
          const o = ot.objects[oi];
                ...
                o.contactsSolid.left = null;
                o.contactsSolid.right = null;
                o.contactsSolid.up = null;
                o.contactsSolid.down = null;

                o.contactsDyn.left = null;
                o.contactsDyn.right = null;
                o.contactsDyn.up = null;
                o.contactsDyn.down = null;

                o.contactsGhost.ghost1 = null;
                o.contactsGhost.ghost2 =null;
                o.contactsGhost.ghost3 =null;
...
const hasDual = (!o.ghost && ((o.rot|0) === 0) && dualPct > 0);
...
let P= { id:uid++, type:'dyn', invMass:1, x:solidX, y:solidY, sx:startX, sy:startY, w:solidW, h:solidH, rot:o.rot||0, ref:o };
    ...
    for (let it=0; it<LOOT_ITER; it++){
      ...
      for (let si=0; si<nearS.length; si++){
       ...
       // G.y -= contact.n.y * depth;
        ...
        if(G._ghostClass !== 'dualFront')if (G.ref) _pushCollisionLog(true,G.ref, (S.ref || S), contact.n, true);
      }
...
collideswiths(obj, name) {
    try {
        return (
            _matchContact(obj.contactsSolid.left, name) ||
            _matchContact(obj.contactsSolid.right, name) ||
            _matchContact(obj.contactsSolid.up, name) ||
            _matchContact(obj.contactsSolid.down, name) ||
            _matchContact(obj.contactsDyn.left, name) ||
            _matchContact(obj.contactsDyn.right, name) ||
            _matchContact(obj.contactsDyn.up, name) ||
            _matchContact(obj.contactsDyn.down, name) ||
            _matchContact(obj.contactsGhost.ghost1, name)||
            _matchContact(obj.contactsGhost.ghost2, name)||
            _matchContact(obj.contactsGhost.ghost3, name)
        );
    } catch (error) {}
...
collideswith(obj, name, dir) {
    try {
        if (dir === "ghost") {  
            return _matchContact(obj.contactsGhost.ghost1, name)||
                   _matchContact(obj.contactsGhost.ghost2, name)||
                   _matchContact(obj.contactsGhost.ghost3, name);
        }
        ...
        return (
            _matchContact(obj.contactsSolid[dir], name) ||
            _matchContact(obj.contactsDyn[dir], name)
        );
    } catch (error) {}
...
collideswithanoterobject(obj, obj2) {
    ...
    addobject(objtype, x, y, dimx, dimy, rot, fliped) {
        ...
        if(objtype.objects[objtype.objects.length-1].isstatic)markStaticsDirty();
    ...
    removeobject(objtype, obj) {
        ...
        if(obj.isstatic)markStaticsDirty();
...
updateUnitMovement(scale) {
    ...
    for (let obj of objects) {
        ...
        const dist = Math.hypot(dx, dy);
                ...
                if (obj.targetBuilding && isCollidingWith(obj, obj.targetBuilding)) {
                    stop = true;
                }
...
function drawSelectRing(ctx, o,zoom, camX, camY){
  ...
  if (o.canMove==false) {
    ...
    if(o.name=="barrack"||o.name=="rbarrack"||o.name=="ybarrack"||o.name=="gbarrack"||o.name=="tower"||o.name=="rtower"||o.name=="ytower"||o.name=="gtower"||o.name=="townhall")cy=o.y+o.dimy*0.50;
...
function _matchContact(ref, name){
  if (!ref) return null;
  if (name === "any") return ref;
  if (ref.name === name) return ref;
  return null;
}
function isCollidingWith(obj, target) {
    return (
        obj.contactsSolid.left === target ||
        obj.contactsSolid.right === target ||
        obj.contactsSolid.up === target ||
        obj.contactsSolid.down === target ||
        obj.contactsDyn.left === target ||
        obj.contactsDyn.right === target ||
        obj.contactsDyn.up === target ||
        obj.contactsDyn.down === target ||
        obj.contactGhost === target
    );
}